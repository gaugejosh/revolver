...
Version: 2.21
*/
define('MEGASUBSCRIBEPOPUP_VERSION', 2.21);
		...
		if (function_exists('mymail_subscribe')) {
			echo '
						<div class="postbox megasubscribepopup_postbox">
							...
							<div class="inside">
								<table class="megasubscribepopup_useroptions">';
			$terms = get_terms('newsletter_lists', array('hide_empty' => false));
			if (sizeof($terms) == 0) {
				echo '
									<tr>
										<th>'.__('Enable MyMail', 'megasubscribepopup').':</th>
										<td>'.__('Please <a href="edit-tags.php?taxonomy=newsletter_lists&post_type=newsletter">create</a> at least one list.', 'megasubscribepopup').'</td>
									</tr>';
			} else {
				echo '
									<tr>
				...
				foreach ($terms as $list) {
					echo '
												<option value="'.$list->term_id.'"'.($list->term_id == $this->options['mymail_listid'] ? ' selected="selected"' : '').'>'.$list->name.'</option>';
				}
				echo '
											</select>
											<br /><em>'.__('Select your List ID.', 'megasubscribepopup').'</em>
...
<body>
	...
	function megasubscribepopup_submit() {
		...
		if (sizeof($errors) == 0) {
			...
			if (function_exists('mymail_subscribe')) {
				if ($this->options['mymail_enable'] == 'on') {
					$term = get_term_by('id', $this->options['mymail_listid'], 'newsletter_lists');
					if (!empty($term)) {
						try {
							$result = mymail_subscribe($email, array('firstname' => $name), array($term->slug), false);
						} catch (Exception $e) {
						}
					}
				}
